...
class Mapper {
    ...
    public function publish_coupon($imported_coupon_id) {
        ...
        if ($status !== 'approved') {
            throw new \Exception(__('Apenas cupons aprovados podem ser publicados', '7k-coupons-importer'));
        }
        ...
        if (!empty($category_terms)) {
            wp_set_object_terms($coupon_id, $category_terms, 'coupon-category', false);
        }
