...
class CurationAdmin {
    ...
    private function get_coupons_query($args = array()) {
        $defaults = array(
            ...
            'post_status' => 'any', // Mudança aqui: buscar todos os status
            'posts_per_page' => 20,
            'paged' => 1,
            'meta_query' => array(),
            's' => ''
    ...
    public function ajax_approve_coupon() {
        ...
        update_post_meta($coupon_id, '_ci7k_approved_at', current_time('mysql'));
        update_post_meta($coupon_id, '_ci7k_approved_by', get_current_user_id());

        $core = \CouponImporter\Core::get_instance();
        $core->get_logger()->log('approve', sprintf(__('Cupom aprovado: %s (ID: %d)', '7k-coupons-importer'), $post->post_title, $coupon_id));

        wp_send_json_success(array('message' => __('Cupom aprovado com sucesso', '7k-coupons-importer')));
    }
    ...
    public function ajax_rewrite_title() {
        ...
        try {
            ...
            $queue->add($coupon_id, 'rewrite_title');
            
            wp_send_json_success(array('message' => __('Reescrita de título adicionada à fila', '7k-coupons-importer')));
    ...
    public function ajax_rewrite_description() {
        ...
        try {
            ...
            $queue->add($coupon_id, 'rewrite_description');
            
            wp_send_json_success(array('message' => __('Reescrita de descrição adicionada à fila', '7k-coupons-importer')));
    ...
    private function process_bulk_ai_rewrite($coupon_ids, $action_type) {
        ...
        if (strpos($prompt, '%s') === false) {
            throw new \Exception(__('O prompt deve conter o marcador %s para inserir a lista de cupons.', '7k-coupons-importer'));
        }
        ...
        try {
            ...
            $rewritten_lines = array_filter($rewritten_lines, function($line) {
    ...
    private function call_openai_bulk($prompt, $settings) {
        ...
        $body = array(
            ...
            'messages' => array(
                array(
                    'role' => 'system',
                    'content' => 'Você é um especialista em marketing de afiliados e copywriting para cupons de desconto. Reescreva os textos de forma profissional e atraente em português brasileiro.'
                ),
                array(
                    'role' => 'user',
                    'content' => $prompt
    ...
    private function call_gemini_bulk($prompt, $settings) {
        ...
        $system_instruction = 'Você é um especialista em marketing de afiliados e copywriting para cupons de desconto. Reescreva os textos de forma profissional e atraente em português brasileiro.';
